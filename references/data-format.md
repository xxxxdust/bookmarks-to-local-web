...
- `posts[]`: `id` 正整数（组内唯一、不必连续）；字符串字段 `title, author, category, url, lead, action, resources, status`；`points` 为重点字符串数组；`prompts, skills` 为对应ID数组。`status` 说明实际读到哪些内容，`resources` 说明资源状况和缺失。
